...
export class HttpWithObservablesExampleComponent implements OnInit, OnDestroy {
  ...
  searchResults2: Observable<SearchItem[]>;
  ...
  ngOnInit(): void {
    ...
    this.searchResults2 = this.searchField.valueChanges.pipe(
      debounceTime(400),
      distinctUntilChanged(),
      ...
      switchMap((searchText: string) => {
        ...
        return this.searchService
          .handleSearchAsObservableJsonP(searchText)
          .pipe(
            catchError((error) => {
  ...
  handleSearch(searchTerm: string): void {
    ...
    this.subscription = this.searchService
      .handleSearchAsObservable(searchTerm)
      .subscribe(
        (response) => {
          this.isSearchLoading = false;
